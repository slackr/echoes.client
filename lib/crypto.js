...
EchoesCrypto.prototype.export_key = function(keychain) {
    ...
    return crypto.subtle.exportKey('jwk', key)
        .then(function(k) {
            ...
            if (keychain == 'keyx_public') {
                exported.public_key.key_ops = ['deriveKey']; // this doesn't get exported for some reason
            }
...
EchoesCrypto.prototype.is_browser_ec_compatible = function() {
    var ua = navigator.userAgent.split(/[\s|\/]/g);

    for (var browser in this.ec_browser_support) {
        var version = parseFloat(ua[ua.indexOf(browser) + 1]);
        if (! isNaN(version)) {
            var is_compatible = (version >= this.ec_browser_support[browser]);
            this.log("browser '" + browser + "' (" + version + ") is compatible with EC: " + is_compatible + " (min: " + this.ec_browser_support[browser] + ")", 0);
            return is_compatible;
        }
    }

    this.log("unknown browser marked as incompatible with EC", 0);
    return false;
}
